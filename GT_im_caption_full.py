...
from GT_input_pipeline import input_fn
from misc_fn import crop_sentence
from misc_fn import get_len
from misc_fn import relation_rewards
...
tf.flags.DEFINE_integer('max_steps', 5001, 'maximum training steps')
...
tf.flags.DEFINE_float('w_obj', 100, 'object weight')
...
def generator(inputs, is_training=True):
  ...
  embedding = tf.get_variable(
    name='embedding',
    shape=[FLAGS.vocab_size, FLAGS.emb_dim],   # 18669 512
    initializer=tf.random_uniform_initializer(-0.08, 0.08))
  softmax_w = tf.matrix_transpose(embedding)
  softmax_b = tf.get_variable('softmax_b', [FLAGS.vocab_size])

  inputs = inputs[0]     # net / CNN features (batch_size * net)
  ...
  for t in range(FLAGS.max_caption_length):   #20
    if t == 0:
    ...
    logit = tf.nn.bias_add(tf.matmul(rnn_out, softmax_w), softmax_b)    #是否把embedding放进去一起训练了
    categorical = tf.contrib.distributions.Categorical(logits=logit)  #预测属于vocab里哪一个words
...
def discriminator(generated_data, generator_inputs, is_training=True):
  ...
  if type(generated_data) is tuple:
    ...
    length = generator_inputs[-1]    #（net, 'len'-1）
  embedding = tf.get_variable(
    name='embedding',
  ...
  mask = tf.sequence_mask(length, tf.shape(sequence)[1])   #sentence 最大长度20个单词中中实际有效的words 去除0
  ...
  feat = slim.fully_connected(state_h, FLAGS.mem_dim, activation_fn=None,
                              scope='recon')
  feat = tf.nn.l2_normalize(feat, axis=1)   #feat 是 reconstruction要用的
  return pred, mask, feat


def rl_loss(gan_model, gan_loss, classes, num_class, relationships, num_rl, objects, add_summaries):
  ...
  o_rewards = relation_rewards(sequence, mask, classes, num_class, relationships, num_rl, objects) * FLAGS.w_obj
  ...
  baselines += relation_rewards(seq_bl, mask, classes, num_class, relationships, num_rl, objects) * FLAGS.w_obj
...
def model_fn(features, labels, mode, params):
  ...
  net = tf.squeeze(net, [1, 2])    #CNN feature
  inc_saver = tf.train.Saver(tf.global_variables('InceptionV4'))
  # inc_saver = tf.train.import_meta_graph("/content/workspace/ckpt/inception_v4.ckpt")
  ...
  gan_loss = rl_loss(gan_model, gan_loss, features['classes'],
                     features['num_class'], features['relationships'], features['num_rl'], features['objects']
                     add_summaries=is_chief)
